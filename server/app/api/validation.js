...
router.get('/validStudent/:query', function (req, res) {
    Student.find({userID: req.params.query}, function (err, result) {
...
router.get('/validCv/:query', function (req, res) {
  ...
  Cv.find({userID: req.params.query}, function (err, result) {
    if (err) return next(err);
    if (result && result.length > 0) {
      isValid = true;
    }
    Student.find({userID: req.params.query}, function (err, result) {
      ...
      res.json({success: true, isValid: isValid, isValidStudent:isValidStudent});
    });
  });
});
